"""
Initializes and runs the backup or recovery process in a separate process.
Sends progress updates back to the main process via a Pipe.
"""
...
def refresh_status(
    ...
    """
    Sends a Progress object through the pipe to update the main process on the
    current status.
    """

    pipe.send(
        Progress(
            current_file, current_file_nr, total_file_nr, skipped, description
        )
    )
...
def write_meta(
    ...
    """
    Writes metadata about the backup process to a file.
    The metadata includes:
    - created: timestamp when the backup was created
    - finished: timestamp when the backup was finished
    - files: number of files copied
    - folder: the originating path of the backup
    - lock: whether the backup is still in progress
    """

    with open(
        path.join(destination_path, destination_folder, ".lorean_meta"), "w"
    ) as f:
        json.dump(
            {
                "created": created,
...
def get_current_date():
    """
    Gets the current date and time formatted as "YYYY-MM-DD HH-MM".
    """
...
def backup(p, logger: Logger, config: dict):
    ...
    for idx, regex in enumerate(regexes):
        ...
        try:
            ...
            error_msg = (
                f"error: invalid exclude regex at line {idx + 1}: '{regex}'"
            )
    ...
    total_files = sum(len(_files) for _, _, _files in walk(originating_path))
    ...
    mkdir(path.join(destination_path, destination_folder))
    ...
    for path_, dirs, files in walk(originating_path):
        ...
        for dir in dirs:
            mkdir(
                path.join(destination_path, destination_folder, rel_path, dir)
            )
        ...
        for file in files:
            ...
            copy(
                ...
                path.join(
                    destination_path, destination_folder, rel_path, file
                ),
            )
...
def recover(p, logger: Logger, config: dict):
    """
    Runs the recovery process in a separate process.
    Sends progress updates back to the main process via a Pipe.
    """
    ...
    logger.debug("Counting amount of files")
    total_files = (
        sum(len(_files) for _, _, _files in walk(originating_path)) - 1
    )  # for the meta file
...
def backup_wrapper(func, childc, config):
    """
    Wrapper function to run the backup or recovery function with error handling
    and logging.
    """

    logger = create_logger(
        f"backup {get_current_date()}",
        path.join(
            LOGDIR,
            f"{get_current_date().replace(' ', '_')}_{str(uuid4())[:8]}.log",
        ),
    )
...
def _create_process(func, config):
    """
    Creates and starts a separate process to run the given function (backup or
    recovery) with the provided configuration. Returns the parent end of the
...
def start_backup(config):
    """
    Starts the backup process in a separate process and returns the parent end
    of the Pipe for communication.
    """
...
def start_recovery(config):
    """
    Starts the recovery process in a separate process and returns the parent
    end of the Pipe for communication.
    """
